...
(() => {
  ...
  function renderCurrent() {
    ...
    const stickyCity = $('stickyWxCity');
    const stickyTemp = $('stickyWxTemp');
    if (stickyCity) stickyCity.textContent = w.city;
    ...
    if (stickyTemp) stickyTemp.textContent = fmtTemp(w.tempF);
  ...
  function initStickyHeader() {
    ...
    const update = () => {
      queued = false;
      const hasWeather = Boolean(state.weather) && !$('weatherPanels').hidden;
      const heroIsBehindHeader = hero.getBoundingClientRect().bottom <= header.offsetHeight;
      header.classList.toggle('header-condensed', hasWeather && heroIsBehindHeader);
    };
    const schedule = () => {
      if (queued) return;
      queued = true;
